#!/usr/bin/env python
# ---
#this file has been changed by m01. See below for the original copyright notice
# ---
#
...
class SimpleCRUD:
  ...
  def _PromptForSpreadsheet(self):
    ...
    print "The following spreadsheets were found in your google docs account:"
    feed = self.gd_client.GetSpreadsheetsFeed()
    self._PrintFeed(feed)
    input = raw_input('\nSelection: ')
    id_parts = feed.entry[string.atoi(input)].id.text.split('/')
    self.curr_key = id_parts[len(id_parts) - 1]
    print "spreadsheet key:",self.curr_key
  
  def _PromptForWorksheet(self):
    # Get the list of worksheets
    print "The following worksheets were found in the selected spreadsheet:"
    feed = self.gd_client.GetWorksheetsFeed(self.curr_key)
    self._PrintFeed(feed)
    input = raw_input('\nSelection: ')
    id_parts = feed.entry[string.atoi(input)].id.text.split('/')
    self.curr_wksht_id = id_parts[len(id_parts) - 1]
    print "worksheet id: ",self.curr_wksht_id
